...
import HeaderForPatientsDiagnosis from './HeaderForPatientsDiagnosis';
import moment from 'moment';
import GroupByDateChart from '../common/GroupByDateChart';

/**
 * Display the result of List of Patients for a diagnosis report
 */
class ListOfPatientsForDiagnosis extends Component {
  ...
  bkBtnClick() {
    let redirectAddress =  '../ListOfDiagnosis';
  ...
  getReportParameters() {
    ...
    if(reportParameters) {
      reportParameters.diagnosisUuid = this.props.match.params.diagnosisUuid;
      reportParameters.diagnosis = this.props.location.state.diagnosis;
      return reportParameters;
    }
    else {
      // if the states are not available then redirect to ListOfDiagnosis report to skip the view failures.
      this.props.history.push({
        pathname: "../ListOfDiagnosis"
      });
      location.reload();
    }
  ...
  render() {
    return (
      <div>
        <ReportTitle heading="Patients affected by Diagnosis" />
        <HeaderForPatientsDiagnosis
...
export default ListOfPatientsForDiagnosis;